...
abstract class ModuleCore
{
    ...
    public static function getModulesOnDisk($use_config = false, $logged_on_addons = false, $id_employee = false)
    {
        ...
        foreach ($module_list as $key => &$module) {
            ...
            } else if (isset($modules_installed[$module->name])) {
    ...
    public static function getModulesDirOnDisk()
    {
        ...
        foreach ($modules as $name) {
            if (is_file(_PS_MODULE_DIR_ . $name)) {
                continue;
            } else if (is_dir(_PS_MODULE_DIR_ . $name . DIRECTORY_SEPARATOR) && Tools::file_exists_cache(_PS_MODULE_DIR_ . $name . '/' . $name . '.php')) {
    ...
    final public static function isModuleTrusted($module_name)
    {
        ...
        if (stripos($trusted_modules_list_content, $module_name) !== false) {
            ...
            if (stripos($modules_list_content, '<module name="' . $module_name . '"/>') == false) {
                return 1;
            } else if (stripos($default_country_modules_list_content, '<name><![CDATA[' . $module_name . ']]></name>') !== false) {
            ...
            return 2;
        } else if (stripos($untrusted_modules_list_content, $module_name) !== false) {
    ...
    public static function getPaymentModules()
    {
        ...
        if (isset($context->employee)) {
            $frontend = false;
        } else if (isset($context->customer) && $use_groups) {
    ...
    protected static function _isTemplateOverloadedStatic($module_name, $template)
    {
        ...
        } else if (Tools::file_exists_cache(_PS_THEME_DIR_ . 'modules/' . $module_name . '/views/templates/hook/' . $template)) {
            return _PS_THEME_DIR_ . 'modules/' . $module_name . '/views/templates/hook/' . $template;
        } else if (Tools::file_exists_cache(_PS_THEME_DIR_ . 'modules/' . $module_name . '/views/templates/front/' . $template)) {
            return _PS_THEME_DIR_ . 'modules/' . $module_name . '/views/templates/front/' . $template;
        } else if (Tools::file_exists_cache(_PS_PARENT_THEME_DIR_ . 'modules/' . $module_name . '/' . $template)) {
            return _PS_PARENT_THEME_DIR_ . 'modules/' . $module_name . '/' . $template;
        } else if (Tools::file_exists_cache(_PS_PARENT_THEME_DIR_ . 'modules/' . $module_name . '/views/templates/hook/' . $template)) {
            return _PS_PARENT_THEME_DIR_ . 'modules/' . $module_name . '/views/templates/hook/' . $template;
        } else if (Tools::file_exists_cache(_PS_PARENT_THEME_DIR_ . 'modules/' . $module_name . '/views/templates/front/' . $template)) {
            return _PS_PARENT_THEME_DIR_ . 'modules/' . $module_name . '/views/templates/front/' . $template;
        } else if (Tools::file_exists_cache(_PS_MODULE_DIR_ . $module_name . '/views/templates/hook/' . $template)) {
            return false;
        } else if (Tools::file_exists_cache(_PS_MODULE_DIR_ . $module_name . '/views/templates/front/' . $template)) {
            return false;
    ...
    /**
     * Use this method to return the result of a smarty template when assign data only locally with
     * $this->smarty->assign()
     *
    ...
    public function getTemplatePath($template)
    {
        ...
        if ($overloaded) {
            return $overloaded;
        } else if (Tools::file_exists_cache(_PS_MODULE_DIR_ . $this->name . '/views/templates/hook/' . $template)) {
            return _PS_MODULE_DIR_ . $this->name . '/views/templates/hook/' . $template;
        } else if (Tools::file_exists_cache(_PS_MODULE_DIR_ . $this->name . '/views/templates/front/' . $template)) {
            return _PS_MODULE_DIR_ . $this->name . '/views/templates/front/' . $template;
        } else if (Tools::file_exists_cache(_PS_MODULE_DIR_ . $this->name . '/' . $template)) {
    ...
    public function removeOverride($classname)
    {
        ...
        if ($orig_path) {
            ...
            for ($i = 0; $i < $count; ++$i) {
                ...
                } else if (preg_match('/(^\s*\/\*)/i', $override_file[$i])) {
